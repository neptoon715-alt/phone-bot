...
PHONES = {
    ...
    "secret": [
        ...
        {"name": "Nokia 7700 (отменён)", "price": 56250, "img": None},
...
def shop(message):
    ...
    for rarity in ["common", "rare", "legendary", "mythic", "secret"]:
        ...
        keyboard.add(telebot.types.InlineKeyboardButton(
            f"{emoji} {rarity_name}",
            callback_data=f"shop_{rarity}"
        ))
    bot.reply_to(message, "🛒 *Магазин телефонов!*\nВыбери категорию:",
                 reply_markup=keyboard, parse_mode="Markdown")


@bot.callback_query_handler(func=lambda call: call.data.startswith("shop_") and not call.data.startswith("shop_buy_"))
def show_shop_rarity(call):
    ...
    for phone in PHONES[rarity]:
        ...
        keyboard.add(telebot.types.InlineKeyboardButton(
            f"{emoji} {phone['name']} — {shop_price} монет",
            callback_data=f"shop_buy_{phone['name']}"
        ))
...
def buy_phone(call):
    ...
    if found_phone:
        ...
        caption = (
            f"🛒 *Куплено!*\n\n"
            f"{emoji} *{found_phone['name']}*\n"
            f"💸 Потрачено: *{shop_price} монет*\n"
            f"💰 Остаток: *{player['balance']} монет*"
        )
